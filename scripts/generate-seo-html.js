...
const STATIC_PAGES = {
  ...
  '/contact': {
    ...
    description: 'Leave your number and Parsley Moving will call you back right away. Available 7 days a week, 9am–9pm across the Bay Area.',
    canonical: 'https://parsleymoving.com/contact',
    noindex: true,
  },
  '/quote': {
...
function generateHtml(html, { title, description, canonical, noindex = false }) {
  ...
  // Replace robots meta for noindex pages
  if (noindex) {
...
for (const [route, seo] of Object.entries(STATIC_PAGES)) {
  if (route === '/') {
    // Home page — overwrite dist/index.html directly
    writeFileSync(resolve(distDir, 'index.html'), generateHtml(template, seo))
    console.log('✅ / (home)')
  } else {
    // Other static pages — create subfolder with index.html
